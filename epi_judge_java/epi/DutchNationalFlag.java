...
public class DutchNationalFlag {
  ...
  public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
    ...
    int lowPtr = 0, middlePtr = 0, highPtr = A.size() - 1;
    int pivotVal = A.get(pivotIndex).ordinal();
    while (middlePtr <= highPtr) {
      int valAtMiddlePtr = A.get(middlePtr).ordinal();
      if (valAtMiddlePtr > pivotVal) {
        Collections.swap(A, middlePtr, highPtr--);
      } else if (valAtMiddlePtr < pivotVal) {
        Collections.swap(A, lowPtr++, middlePtr++);
      } else {
        middlePtr++;
      }
    }
